...
@section('header', 'License Management')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-800">Licenses</h2>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 p-6">
        ...
        <form id="license-filter-form" method="GET" action="{{ route('licenses.index') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <!-- Search -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Search</label>
                    <input type="text" id="license-search" name="search" value="{{ request('search') }}" placeholder="Search by software name or key..." 
                ...
                <div>
                    ...
                    <select id="license-status-filter" name="status" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        ...
                        <option value="expired_soon" {{ request('status') == 'expired_soon' ? 'selected' : '' }}>Expired Soon</option>
                    </select>
                </div>
            ...
            <div class="pt-2">
                <a href="{{ route('licenses.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                ...
                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700" id="licenses-tbody">
                    @forelse($licenses as $license)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-gray-900 dark:text-gray-200">{{ $license->software_name }}</div>
                                <div class="text-xs text-gray-500 dark:text-gray-400">{{ Str::limit($license->license_key, 20) }}...</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-gray-200">{{ $license->department?->name ?? 'N/A' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-gray-200">{{ $license->quantity ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3 flex justify-end">
                                <a href="{{ route('licenses.show', $license) }}" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300 transition" title="View Details">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path>
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path>
                                    </svg>
                                </a>
                                <a href="{{ route('licenses.edit', $license) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 transition" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                </a>
                                <form action="{{ route('licenses.destroy', $license) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this license?')">
                                    @csrf
                                    @method('DELETE')
...
<script>
    ...
    function applyLicenseFilters() {
        ...
        fetch(`{{ route('licenses.index') }}?${params.toString()}`, {
            headers: {
        ...
        .then(html => {
            ...
            window.history.replaceState({}, '', `{{ route('licenses.index') }}?${params.toString()}`);
        })
        .catch(error => {
    ...
    function openLicenseModal() {
        ...
        const depts = {!! json_encode($departments->map(fn($d) => ['id' => $d->id, 'name' => $d->name])) !!};
        ...
        content.innerHTML = `
            <div class="p-6">
                ...
                <form action="{{ route('licenses.store') }}" method="POST" class="space-y-4">
                    ...
                    <div class="flex gap-2 mt-6">
                        ...
                        <button type="button" onclick="closeLicenseModal()" class="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-900 dark:text-gray-200 rounded-md">Cancel</button>
                    </div>
                </form>
            </div>
        `;
