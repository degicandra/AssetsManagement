...
@section('header', 'Manage Locations')
...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 p-6">
        ...
        <form id="location-filter-form" method="GET" action="{{ route('locations.index') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <select name="floor_id" id="location-floor-filter" class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
                        ...
                        @foreach($floors as $floor)
                            <option value="{{ $floor->id }}" {{ request('floor_id') == $floor->id ? 'selected' : '' }}>Floor {{ $floor->floor_number }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="pt-2">
                <a href="{{ route('locations.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-600 transition">
                    ...
                    @forelse($locations as $location)
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900 dark:text-gray-200">{{ $location->name }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-gray-200">{{ $location->floor ? 'Floor ' . $location->floor->floor_number : '-' }}</div>
                            </td>
                            <td class="px-6 py-4 max-w-md">
                                <div class="text-sm text-gray-600 dark:text-gray-400">{{ $location->description ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600 dark:text-gray-400">{{ $location->created_at->format('M d, Y') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-2">
                                <button onclick='openEditLocationModal({!! json_encode($location) !!})' class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 font-medium">Edit</button>
...
<script>
    ...
    function applyLocationFilters() {
        ...
        fetch(`{{ route('locations.index') }}?${params.toString()}`, {
            headers: {
        ...
        .then(html => {
            ...
            window.history.replaceState({}, '', `{{ route('locations.index') }}?${params.toString()}`);
        })
        .catch(error => {
    ...
    const floorsData = {!! json_encode($floors) !!};
