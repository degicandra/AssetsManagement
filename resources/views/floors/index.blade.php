            ...
            <table class="w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700" id="floor-table-body">
                    @forelse($floors as $floor)
                        <tr class="floor-row hover:bg-gray-50 dark:hover:bg-gray-700 transition" data-name="{{ strtolower($floor->name ?? '') }}" data-number="{{ $floor->floor_number }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900 dark:text-gray-200">{{ $floor->floor_number }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-gray-200">{{ $floor->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 max-w-md">
                                <div class="text-sm text-gray-600 dark:text-gray-400">{{ $floor->description ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm space-x-2">
                                <button onclick='openEditFloorModal({!! json_encode($floor) !!})' class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300 font-medium">Edit</button>
...
<script>
    ...
    function openFloorModal() {
        ...
        document.getElementById('floor-form').action = `{{ route('floors.store') }}`;
